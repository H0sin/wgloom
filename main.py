...
from config import DEBUG, UVICORN_SSL_CA_TYPE, UVICORN_SSL_CERTFILE, UVICORN_SSL_KEYFILE, UVICORN_HOST, UVICORN_PORT
...
You're running wgloom with: {click.style('UVICORN_SSL_CA_TYPE', italic=True, fg="magenta")}: {click.style(f'{ca_type}', bold=True, fg="yellow")}. 
...
if __name__ == "__main__":
    ...
    if UVICORN_SSL_CERTFILE and UVICORN_SSL_KEYFILE and UVICORN_SSL_CA_TYPE:
        ...
        logger.warning(f"""
    {click.style('IMPORTANT!', blink=True, bold=True, fg="yellow")}
    You're running wgloom without specifying {click.style('UVICORN_SSL_CERTFILE', italic=True, fg="magenta")} and {click.style('UVICORN_SSL_KEYFILE', italic=True, fg="magenta")}.
    The application will only be accessible through localhost. This means that {click.style('wgloom and subscription URLs will not be accessible externally', bold=True)}.
